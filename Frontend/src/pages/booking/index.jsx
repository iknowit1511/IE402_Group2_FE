...
function Booking() {
  ...
  // Nhận dữ liệu từ location.state.bookingData
  const bookingData = location.state?.bookingData || {};
  const tourData = bookingData.tour || {};
  const selectedDate = bookingData.selectedDate || "";
  const guestCount = bookingData.guestCount || 1;

  // Fallback từ URL params nếu không có state
  const maTour = searchParams.get("maTour") || tourData.id || "TTH-xxxxx";
  const tenTour =
    searchParams.get("tenTour") || tourData.title || "Du lịch Bạc Liêu – Dấu Ấn Miền Tây";
  ...
  const prices = {
    ...
    treEm: tourData.price ? Math.floor(tourData.price * 0.6) : 1200000,
    emBe: tourData.price ? Math.floor(tourData.price * 0.15) : 300000,
  };

  // Kiểm tra nếu không có dữ liệu tour
  useEffect(() => {
    if (!bookingData || !tourData.id) {
      ...
      if (!savedData) {
        ...
        // Parse dữ liệu từ sessionStorage nếu cần
        const parsedData = JSON.parse(savedData);
        setFormData((prev) => ({
          ...prev,
          ngayKhoiHanh: parsedData.selectedDate || prev.ngayKhoiHanh,
        }));
        setSoLuong((prev) => ({
          ...prev,
          nguoiLon: parsedData.guestCount || prev.nguoiLon,
        }));
      }
    }
  }, [bookingData, tourData, navigate]);
  ...
  const handleSubmit = () => {
    ...
    sessionStorage.setItem("tourBookingData", JSON.stringify(data));
    navigate("/payment", {
      state: {
        formData,
        soLuong,
    ...
    <div className="container">
          ...
          <div className="form-subgroup">
            ...
            <div className="form-group">
              ...
              <input
                ...
                readOnly // Để ngăn người dùng sửa ngày khởi hành
              />
            </div>
          </div>
        </div>
      ...
      <div className="summary-section">
        <div className="summary-box">
          ...
          <p>Thời gian: {tourData.duration || "3N2Đ"}</p>
          <p>
            Người lớn:{" "}
            <strong>
